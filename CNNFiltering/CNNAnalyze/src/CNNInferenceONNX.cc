...
private:
  ...
  std::ofstream doubletsNumOutput;
...
void CNNInferenceONNX::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  ...
  inferenceTimeOutput.open("doublets/inferenceTimeONNX_" + std::to_string(intraThreads) + "_" + std::to_string(interThreads) + ".txt", std::ofstream::app);
  inferenceCPUTimeOutput.open("doublets/inferenceCPUTimeONNX_" + std::to_string(intraThreads) + "_" + std::to_string(interThreads) + ".txt", std::ofstream::app);
  doubletsNumOutput.open("doublets/doubletsNum.txt", std::ofstream::app);
  ...
  for (std::vector<IntermediateHitDoublets::LayerPairHitDoublets>::const_iterator lIt = iHd->layerSetsBegin(); lIt != iHd->layerSetsEnd(); ++lIt)
  {
    ...
    totalDoublets += numOfDoublets;
    doubletsNum += numOfDoublets;
    doubletsNumOutput << doubletsNum << std::endl;
    if (totalDoublets > maxBatchSize)
      continue;
    inputs[0].resize(totalDoublets * padSize * padSize * cnnLayers * 2);
    inputs[1].resize(totalDoublets * infoSize);
    ...
    for (size_t i = 0; i < lIt->doublets().size(); i++)
    {
      ...
      for (int jc = 0; jc < cnnLayers; jc++)
      {
        for (int nx = 0; nx < padSize * padSize; nx++)
        {
          vPad[nx + padOffset] = inHitPads[jc][nx];
        }
        padOffset += padSize * padSize;
      }
      ...
      for (int jc = 0; jc < cnnLayers; jc++)
      {
        for (int nx = 0; nx < padSize * padSize; nx++)
        {
          vPad[nx + padOffset] = outHitPads[jc][nx];
        }
        padOffset += padSize * padSize;
      }
    ...
    if ((!allLayers) | (lIt == iHd->layerSetsEnd()))
    {
      ...
      model->run(input_names, inputs, output_names, totalDoublets);
      ...
      inferenceTimeOutput << totalDoublets << " " << std::fixed << std::setprecision(4) << t_duration << std::endl;
      inferenceCPUTimeOutput << totalDoublets << " " << std::fixed << std::setprecision(4) << c_duration << std::endl;
      std::cout << totalDoublets << " Wall Clock Time:" << std::fixed << std::setprecision(4) << t_duration << std::endl;
      std::cout << totalDoublets << " CPU Time:" << std::fixed << std::setprecision(4) << c_duration << std::endl;
    }
    lIt--;
