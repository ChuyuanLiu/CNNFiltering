...
//
CNNInference::CNNInference(const edm::ParameterSet &iConfig) : processName_(iConfig.getParameter<std::string>("processName")),
...
void CNNInference::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  using namespace edm;
  ...
  for (std::vector<IntermediateHitDoublets::LayerPairHitDoublets>::const_iterator lIt = iHd->layerSetsBegin(); lIt != iHd->layerSetsEnd(); ++lIt)
  {
    ...
    if (find(pixelDets.begin(), pixelDets.end(), innerLayer->seqNum()) == pixelDets.end())
      continue; //TODO change to std::map ?

    DetLayer const *outerLayer = lIt->doublets().detLayer(HitDoublets::outer);
    if (find(pixelDets.begin(), pixelDets.end(), outerLayer->seqNum()) == pixelDets.end())
      continue;
    ...
    if (numOfDoublets > maxBatchSize)
      continue;
    std::shared_ptr<tensorflow::Tensor> inputPads;
    std::shared_ptr<tensorflow::Tensor> inputFeat;

    inputPads = std::make_shared<tensorflow::Tensor, tensorflow::DataType, const tensorflow::TensorShape &>(
        tensorflow::DT_FLOAT, {numOfDoublets, padSize, padSize, cnnLayers * 2});
    inputFeat = std::make_shared<tensorflow::Tensor, tensorflow::DataType, const tensorflow::TensorShape &>(
        tensorflow::DT_FLOAT, {numOfDoublets, infoSize});
    ...
    for (size_t i = 0; i < lIt->doublets().size(); i++)
    {
      ...
      for (int j = 0; j < 2; ++j)
      {
        ...
        deltaA -= ((float)clusters[j]->size());
        deltaA *= -1.0;
        deltaADC -= clusters[j]->charge();
        deltaADC *= -1.0; //At the end == Outer Hit ADC - Inner Hit ADC
        deltaS -= ((float)(clusters[j]->sizeY()) / (float)(clusters[j]->sizeX()));
        deltaS *= -1.0;
        deltaR -= lIt->doublets().r(i, layers[j]);
        deltaR *= -1.0;
        deltaPhi -= phi;
        deltaPhi *= -1.0;
      }
      ...
      for (int jc = 0; jc < cnnLayers; jc++)
      {
        thisOffset = jc * padSize * padSize + cnnLayers * padSize * padSize;
        for (int nx = 0; nx < padSize * padSize; nx++)
        {
    ...
    inferenceTimeOutput << numOfDoublets << " " << std::fixed << std::setprecision(4) << t_duration << std::endl;
    inferenceCPUTimeOutput << numOfDoublets << " " << std::fixed << std::setprecision(4) << c_duration << std::endl;
    std::cout << numOfDoublets << " Wall Clock Time:" << std::fixed << std::setprecision(4) << t_duration << std::endl;
    std::cout << numOfDoublets << " CPU Time:" << std::fixed << std::setprecision(4) << c_duration << std::endl;
...
void CNNInference::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void CNNInference::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void CNNInference::fillDescriptions(edm::ConfigurationDescriptions &descriptions)
{
