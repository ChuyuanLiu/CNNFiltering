	...
	//nrecHit_vs_nsimHit_rec2sim[w]->Fill(track->numberOfValidHits(), (int)(simhits.end()-simhits.begin() ));
      ...
      for (size_t k = 0; k < theDoublets.size(); ++k)
      {
        ...
        for (std::vector<IntermediateHitDoublets::LayerPairHitDoublets>::const_iterator lIt= (*iHd).layerSetsBegin(); lIt != (*iHd).layerSetsEnd(); ++lIt)
        {
          ...
          std::vector< float > inTP, outTP, theTP,theIntersection;
          ...
          for (size_t i = 0; i < lIt->doublets().size(); i++)
          {
            ...
            if(kIntersection.size()>0)
            {
              ...
              theIntersection.push_back(1.0);
              theIntersection.push_back(kIntersection.size());
              ...
              theIntersection.push_back(iParticle);

              theIntersection.push_back(1.0); // 1
              theIntersection.push_back(kPar->second.key()); // 2
              theIntersection.push_back(momTp.x()); // 3
              theIntersection.push_back(momTp.y()); // 4
              theIntersection.push_back(momTp.z()); // 5
              theIntersection.push_back(particle.pt()); //6

              theIntersection.push_back(particle.mt());
              theIntersection.push_back(particle.et());
              theIntersection.push_back(particle.massSqr()); //9

              theIntersection.push_back(particle.pdgId());
              theIntersection.push_back(particle.charge()); //11

              theIntersection.push_back(particle.numberOfTrackerHits()); //TODO no. pixel hits?
              theIntersection.push_back(particle.numberOfTrackerLayers());
              //TODO is cosmic?
              theIntersection.push_back(particle.phi());
              theIntersection.push_back(particle.eta());
              theIntersection.push_back(particle.rapidity()); //16

              theIntersection.push_back(verTp.x());
              theIntersection.push_back(verTp.y());
              theIntersection.push_back(verTp.z());
              theIntersection.push_back((-verTp.x()*sin(momTp.phi())+verTp.y()*cos(momTp.phi()))); //dxy
              theIntersection.push_back((verTp.z() - (verTp.x() * momTp.x()+
              verTp.y() *
              momTp.y())/sqrt(momTp.perp2()) *
              ...
              */
              ...
              theIntersection.push_back(particle.eventId().bunchCrossing()); //22
              theIntersection.push_back(1.0); //For compatibility with tracks infos, nothing special.
              theIntersection.push_back(1.0);
              theIntersection.push_back(1.0);
              theIntersection.push_back(1.0);
            ...
            for (size_t i = 0; i < theIntersection.size(); i++)
            outCNNFile << theIntersection[i] << "\t";
