...
MultiTrackValidatorCNNTp::MultiTrackValidatorCNNTp(const edm::ParameterSet& pset):
...
MultiTrackValidatorCNNTp::~MultiTrackValidatorCNNTp() {}


void MultiTrackValidatorCNNTp::bookHistograms(DQMStore::IBooker& ibook, edm::Run const&, edm::EventSetup const& setup) {
...
const TrackingVertex::LorentzVector *MultiTrackValidatorCNNTp::getSimPVPosition(const edm::Handle<TrackingVertexCollection>& htv) const {
...
const reco::Vertex::Point *MultiTrackValidatorCNNTp::getRecoPVPosition(const edm::Event& event, const edm::Handle<TrackingVertexCollection>& htv) const {
...
void MultiTrackValidatorCNNTp::tpParametersAndSelection(const TrackingParticleRefVector& tPCeff,
...
size_t MultiTrackValidatorCNNTp::tpDR(const TrackingParticleRefVector& tPCeff,
...
void MultiTrackValidatorCNNTp::trackDR(const edm::View<reco::Track>& trackCollection, const edm::View<reco::Track>& trackCollectionDr, DynArray<float>& dR_trk) const {
  int i=0;
...
void MultiTrackValidatorCNNTp::analyze(const edm::Event& event, const edm::EventSetup& setup){
	...
	*/
	...
	//nrecHit_vs_nsimHit_rec2sim[w]->Fill(track->numberOfValidHits(), (int)(simhits.end()-simhits.begin() ));
      ...
      for (size_t k = 0; k < theDoublets.size(); ++k)
      {
        ...
        std::string fileName = "doublets_tracks/" + std::to_string(lumNumber) +"_"+std::to_string(runNumber) +"_"+std::to_string(eveNumber) + "_" + dName + "_dnn_doublets.txt";;
        ...
        for (std::vector<IntermediateHitDoublets::LayerPairHitDoublets>::const_iterator lIt= (*iHd).layerSetsBegin(); lIt != (*iHd).layerSetsEnd(); ++lIt)
        {
          ...
          std::vector< float > inTP, outTP, theTrack,theParticle;
          ...
          for (size_t i = 0; i < lIt->doublets().size(); i++)
          {
            ...
            subDetIds.clear(); detSeqs.clear(); hitPars.clear(); theTrack.clear();
            ...
            //Checking Clusters and particles
            ...
            if(kIntersection.size()>0) tpMatched = true;
            ...
            if(tpMatched)
            {
              ...
              theParticle.push_back(kIntersection.size());
              ...
              theParticle.push_back(momTp.z()); // 5
              theParticle.push_back(particle.pt()); //6

              theParticle.push_back(particle.mt());
              theParticle.push_back(particle.et());
              theParticle.push_back(particle.massSqr()); //9

              theParticle.push_back(particle.pdgId());
              theParticle.push_back(particle.charge()); //11

              theParticle.push_back(particle.numberOfTrackerHits()); //TODO no. pixel hits?
              theParticle.push_back(particle.numberOfTrackerLayers());
              //TODO is cosmic?
              theParticle.push_back(particle.phi());
              theParticle.push_back(particle.eta());
              theParticle.push_back(particle.rapidity()); //16

              theParticle.push_back(verTp.x());
              theParticle.push_back(verTp.y());
              theParticle.push_back(verTp.z());
              theParticle.push_back((-verTp.x()*sin(momTp.phi())+verTp.y()*cos(momTp.phi()))); //dxy
              theParticle.push_back((verTp.z() - (verTp.x() * momTp.x()+
              verTp.y() *
              ...
              */
              ...
              theParticle.push_back(particle.eventId().bunchCrossing()); //22
              theParticle.push_back(1.0); //For compatibility with tracks infos, nothing special.
              theParticle.push_back(1.0);
              theParticle.push_back(1.0);
              theParticle.push_back(1.0);
            ...
            bool inTrueOnce = false, outTrueOnce = false;
            ...
            for(View<Track>::size_type i=0; i<trackCollection.size(); ++i){
              ...
              theTrack.push_back(1.0); // 1
              theTrack.push_back((float)(i)); // 2
              theTrack.push_back(momTp.x()); // 3
              theTrack.push_back(momTp.y()); // 4
              theTrack.push_back(momTp.z()); // 5
              theTrack.push_back(particle.pt()); //6

              theTrack.push_back(particle.mt());
              theTrack.push_back(particle.et());
              theTrack.push_back(particle.massSqr()); //9

              theTrack.push_back(particle.pdgId());
              theTrack.push_back(particle.charge()); //11

              theTrack.push_back(particle.numberOfTrackerHits()); //TODO no. pixel hits?
              theTrack.push_back(particle.numberOfTrackerLayers());
              //TODO is cosmic?
              theTrack.push_back(particle.phi());
              theTrack.push_back(particle.eta());
              theTrack.push_back(particle.rapidity()); //16

              theTrack.push_back(verTp.x());
              theTrack.push_back(verTp.y());
              theTrack.push_back(verTp.z());
              theTrack.push_back((-verTp.x()*sin(momTp.phi())+verTp.y()*cos(momTp.phi()))); //dxy
              theTrack.push_back((verTp.z() - (verTp.x() * momTp.x()+
              verTp.y() *
              momTp.y())/sqrt(momTp.perp2()) *
              momTp.z()/sqrt(momTp.perp2())));

              theTrack.push_back(particle.eventId().bunchCrossing()); //22
              theTrack.push_back(isChargeMatched);
              theTrack.push_back(isSigSimMatched);
              theTrack.push_back(sharedFraction);
              theTrack.push_back(numAssocRecoTracks); //26
            ...
            for (size_t i = 0; i < theTrack.size(); i++)
            outCNNFile << theTrack[i] << "\t";

            for (size_t i = 0; i < theParticle.size(); i++)
            outCNNFile << theParticle[i] << "\t";
